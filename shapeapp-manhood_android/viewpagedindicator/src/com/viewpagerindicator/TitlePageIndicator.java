...
public class TitlePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if ((bound.left > left && bound.left < right) || (bound.right > left && bound.right < right)) {
                ...
                String[] dateFragments = pageTitle.toString().split("\\s+");
                ...
                drawHeader(canvas, bound, dateFragments);
    ...
    private static int BigFont = 50;
    private static int SmallFont1;// = -2;
    private static int SmallFont2;// = -5;
    ...
    private void drawHeader(Canvas canvas, Rect bound, String[] dateFragments) {

        SmallFont1 = (int)convertDpToPixel(-2, getContext());//
        SmallFont2 = (int)convertDpToPixel(-5, getContext());//
        String day = dateFragments[0];
        float baseFontSize = mPaintText.getTextSize();
        mPaintText.setTextSize(baseFontSize+BigFont);
        float bigTextMargin = (getHeight() - bound.bottom)/2;
        float offsetLeft = (int) mPaintText.measureText(day, 0, day.length());
        offsetLeft += convertDpToPixel(5, getContext());//

        canvas.drawText(day, 0, day.length(), bound.left, getHeight() - bigTextMargin, mPaintText);
        mPaintText.setTextSize(baseFontSize + SmallFont1);

        //float smallHeight = (int) (mPaintText.descent() - mPaintText.ascent());


        day = dateFragments[1];

        float widthDelta = bound.width() - offsetLeft -  mPaintText.measureText(day, 0, day.length());
        while(widthDelta < 0)
        {
            SmallFont1--;
            mPaintText.setTextSize(baseFontSize + SmallFont1);
            widthDelta = bound.width() - offsetLeft -  mPaintText.measureText(day, 0, day.length());
        }



        Rect textBounds = new Rect();
        mPaintText.getTextBounds(day, 0, day.length(), textBounds);
        float smallHeight = -textBounds.top;
        canvas.drawText(day, 0, day.length(), bound.left + offsetLeft, bigTextMargin + smallHeight, mPaintText);

        int color = mPaintText.getColor();
        mPaintText.setARGB(255, 170, 170, 170);

        mPaintText.setTextSize(baseFontSize + SmallFont2);

        String dayMonth = dateFragments[2] + " " + dateFragments[3];

        widthDelta = bound.width() - offsetLeft -  mPaintText.measureText(dayMonth, 0, dayMonth.length());
        while(widthDelta < 0)
        {
            SmallFont2--;
            mPaintText.setTextSize(baseFontSize + SmallFont2);
        ...
        canvas.drawText(dayMonth, 0, dayMonth.length(), bound.left + offsetLeft, getHeight() - bigTextMargin, mPaintText);

        mPaintText.setTextSize(baseFontSize);
    ...
    private Rect calcBounds(int index, Paint paint) {
        ...
        String[] dateFragments = title.toString().split("\\s+");

        float size = paint.getTextSize();
        paint.setTextSize(size + BigFont);

        //bounds.right = (int) paint.measureText(dateFragments[0] + " ", 0, dateFragments[0].length() + 1);
        Rect textBounds = new Rect();
        paint.getTextBounds(dateFragments[0] + " ", 0, dateFragments[0].length() + 1, textBounds);
        bounds.bottom = -textBounds.top;

        //paint.setTextSize(size - SmallFont1);
        //String dateString = dateFragments[2] + " " + dateFragments[3];
        //bounds.right += (int) paint.measureText(dateString, 0, dateString.length());
        bounds.right = getWidth()/3;
